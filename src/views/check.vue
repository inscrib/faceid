...
async function fnRedrawDiscern() {
  ...
  const dims = faceapi.matchDimensions(discernCanvasEl, discernVideoEl, true);
...
async function fnOpen() {
  ...
  try {
    ...
    stream = mediaStream;
    discernVideoEl.srcObject = mediaStream;
    discernVideoEl.play();
...
function fnClose() {
  ...
  setTimeout(() => {
    discernCanvasEl
      .getContext("2d")
      .clearRect(0, 0, discernCanvasEl.width, discernCanvasEl.height);
  }, 500);
}
